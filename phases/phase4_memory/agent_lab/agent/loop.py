...
class AgentLoop: #初始化了上下文，记忆模块
    ...
    def run(
        ...
        try:
            ...
            self._append_reflection_memory(result)
    ...
    def _build_context_manager_for_run(self, user_goal: str) -> ContextManager:
        recalled_records = self._recall_memory(user_goal)# 回忆记忆，返回了多个memoryrecord
    ...
    def _append_reflection_memory(self, result: AgentRunResult) -> None:
        if self._memory_store is None:
            return
        if result.reflection is None:
            raise ValueError("Cannot save empty reflection to memory")
